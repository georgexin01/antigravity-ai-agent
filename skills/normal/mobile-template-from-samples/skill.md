...
1. **Viewport `width=412`** (Samsung S20 Ultra) on `<meta name="viewport">` — never `device-width`.
...
11. **APEX 0 gate** — never write to `.gemini/knowledge/` or `.gemini/skills/` without explicit per-file user authorization in chat.
...
1. **Genesis scaffold** — `package.json`, `vite.config.ts`, `tsconfig.json`, `postcss.config.js`, `index.html` (viewport=412 + Material Symbols + sample-language fonts via Google Fonts), `.env`, `.env.example`, `.gitignore`.
...
25. Verify viewport=412 in `index.html`, mount target `#root` in main.ts.

### Phase F — Knowledge feedback (APEX 0 GATED)

Per `.gemini/knowledge/0_apex/GROUND_KERNEL.md` Phase 0 (Sovereign Knowledge Freeze), AI is FORBIDDEN from writing to `.gemini/` without per-file authorization. The ASK pattern:
...
| Pitfall | Symptom | Fix |
|---|---|---|
| Using `width=device-width` viewport | Real Samsung S20 doesn't match Chrome F12 | Force `width=412` |
