...
1. **Research user needs & Market Patterns**: Actively search the web for the latest (current year) design templates and layout strategies in the specific industry before starting.
2. **Design structurally rich layouts**: Explicitly avoid basic "div box + icon + text" cards. Utilize advanced patterns like Asymmetrical Grids, Bento Boxes, Split Screens (Image + Content), and overlapping elements.
3. **Design systematically** with tokens and reusable components.
4. **Prioritize accessibility** and inclusive design from concept stage.
5. **Document design decisions** with clear rationale and guidelines.
6. **Collaborate with developers** for optimal implementation.
7. **Test and iterate** based on user feedback and analytics.
8. **Self-Evolve**: After completing a design task, record the newly discovered optimal layout patterns into your knowledge base or skill file.
