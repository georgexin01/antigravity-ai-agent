...
## 🏗️ THE INTELLIGENCE REGISTRY (V2 Patterns)
*Mapped triggers to pre-synthesized skills for instant speed gains.*

| Pattern ID | Trigger (Regex/Schema) | Target Skill | Token ROI | Confidence |
| :--- | :--- | :--- | :--- | :--- |
| **REG-001** | `column: status` | Tag Slot + Color mapping | High (+2.5k) | 95% |
| **REG-002** | `column: salary\|price` | Custom Formatter: RM | High (+1.8k) | 90% |
| **REG-003** | `FK: _id` (1:N) | Relational Drawer Slot | Med (+4.2k) | 85% |
| **REG-004** | New Table (Vben Admin) | **[SKILL-001] RLS Grant** | High (+5.5k) | 99% |

---

## 🧬 IMPROVEMENT SKILLS (Synthesized "Best Ways")
...
## 🔄 TRIGGER RULES (V2.1 - Safety First)
1. **Registry First**: During Phase P1 (Schema Analysis), scan `THE INTELLIGENCE REGISTRY`.
2. **Predictive Suggestion**: If Confidence > 90% → Present Skill as a "Proposed Best Way" in the Consultation Card.
3. **Negative Learning**: If Skill fails Lint/SQL → Confidence -2 → Move to "DEPRECATED" if < 0%.
4. **MANDATORY APPROVAL**: ALL skill executions REQUIRE explicit user approval in the chat. Auto-Execution is strictly DISABLED.
