# 🧭 SKILL PATH ROUTER (V1.0) — TASK ORCHESTRATION

> **Status**: INITIALIZED
> **Identity**: ANTIGRAVITY_ROUTING_ENGINE
> **Goal**: Map user intent to the correct AI Mode and Knowledge Tier.

## 🗺️ ROUTING LOGIC

| Intent Category | Primary Skill Path | Target Mode |
| :--- | :--- | :--- |
| **Cinematic UI / Motion** | `Design Mastery > GSAP Synergy` | NORMAL (Design Master) |
| **CRUD / Architectural** | `Backend Scaffolding > Supabase` | CLAUDE (Logic Architect) |
| **Automation / Earning** | `Vision Pulse > Anti-Bot Bypassing` | FAUCET (Sovereign Earner) |
| **Knowledge Evolution** | `DNA Synthesis > Vault Logging` | CORE (Antigravity Director) |

## 🚦 EXECUTION GATES
1. **Discovery Gate**: Is the project fingerprinted in `evolving_knowledge.md`?
2. **Logic Gate**: Does the task exceed 1,000 tokens? -> Delegate to **Gemma-4**.
3. **Vision Gate**: Is a UI verification needed? -> Trigger **VISION_PULSE (Snipaste)**.

## 🛠️ ACTIVE SKILLSETS
- **Zeta Core 9**: (New) Cinematic scroll orchestration logic.
- **Liquid Glass 1.0**: High-fidelity backdrop-filter specs.
- **Heritage High-Density**: Fast-loading, premium UI boilerplate.

---
_Skill Path Router V1.0 — Antigravity Orchestration Core (2026-04-10)_
